<template>
  <section id="about" class="relative w-full py-32 md:py-48 px-6 md:px-12 bg-[#0D1535] overflow-hidden">
    <!-- Background Image -->
    <div class="absolute inset-0 z-0">
      <img src="https://images.unsplash.com/photo-1543722530-d2c3201371e7?w=1600&q=80" alt="Background"
        class="w-full h-full object-cover opacity-20" />
      <div class="absolute inset-0 bg-gradient-to-r from-[#0D1535] via-[#0D1535]/90 to-[#0D1535]/50" />
    </div>
    ...
    <div class="max-w-7xl mx-auto relative z-10">
      ...
      <div class="mb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <div v-motion :initial="{ opacity: 0, x: -30 }"
          ...
          <div class="flex items-center gap-4 mb-6">
            <img src="../assets/propuestas-logo-n2-07.png" alt="GED 2026"
              class="w-14 h-14 rounded drop-shadow-[0_0_20px_rgba(201,107,53,0.8)]" />
            <span
          ...
          <h2 class="font-orbitron text-4xl lg:text-5xl font-black text-white leading-tight mb-6 uppercase">
            Del desierto <br />
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#C96B35] to-[#4E8FCC]">a la
              oportunidad:</span>
          </h2>
          <p class="font-inter text-base md:text-lg font-light text-[#F0EAD6]/80 leading-relaxed mb-6">
          ...
          <p class="font-inter text-base md:text-lg font-light text-[#F0EAD6]/80 leading-relaxed">
        ...
        <!-- Right Side: Video -->
        <div v-motion :initial="{ opacity: 0, x: 30 }" :visibleOnce="{
          opacity: 1,
          x: 0,
          ...
          <video ref="videoRef" src="../assets/presentacion2.mp4"
            class="relative w-full h-full object-cover z-10 rounded-2xl outline-none" controls playsinline></video>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
        ...
        <div v-for="(pillar, index) in pillars" :key="index" v-motion :initial="{ opacity: 0, y: 30 }" :visibleOnce="{
          opacity: 1,
          y: 0,
          ...
          <div class="mb-6 flex items-center justify-between">
            <div
              ...
              <component :is="pillar.icon" class="text-[#F0EAD6]/70 group-hover:text-[#C96B35] transition-colors"
                :size="28" />
            </div>
            <span
              class="font-mono text-xs text-white/20 uppercase font-bold group-hover:text-[#FFD447]/50 transition-colors">0{{
                index + 1 }}</span>
          </div>
          <h3 class="font-orbitron text-xl font-bold text-[#F0EAD6] mb-3 group-hover:text-[#E8A067] transition-colors">
            {{ pillar.title }}
          </h3>
          <p class="font-inter text-sm text-[#8A9BB8] leading-relaxed flex-grow">
            {{ pillar.desc }}
          </p>
        </div>
      </div>
