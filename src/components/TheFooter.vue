    ...
    <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-12 border-t border-white/10 pt-16">
      <div class="col-span-1 md:col-span-2">
        ...
        <p class="text-[#E6EAF0]/60 max-w-sm mb-6">
          Donde la innovación y la sostenibilidad se encuentran en el corazón de Atacama. Organizado por WakiLabs.
        </p>
        <div class="flex gap-4">
          <a v-for="(social, i) in socials" :key="i" href="#" class="p-2 bg-white/5 rounded-full hover:bg-[#FF6B00] hover:text-white transition-colors">
      ...
      <div>
        <h3 class="font-orbitron text-lg font-bold text-white mb-6">Enlaces Rápidos</h3>
        <ul class="space-y-4 text-[#E6EAF0]/60">
          <li v-for="link in quickLinks" :key="link">
            <a href="#" class="hover:text-[#00F5D4] transition-colors">{{ link }}</a>
          </li>
        </ul>
      </div>
      ...
      <div>
        ...
        <ul class="space-y-4 text-[#E6EAF0]/60">
          <li class="flex items-center gap-2"><Mail :size="16" /> contacto@wakilabs.cl</li>
          <li>Desierto de Atacama, Chile</li>
        </ul>
        <button class="mt-6 px-6 py-3 bg-[#FF6B00] text-white font-orbitron font-bold text-sm rounded hover:bg-[#FFC857] hover:text-black transition-colors">
          Suscribirse al Newsletter
        </button>
      </div>
    </div>

    <div class="max-w-7xl mx-auto px-6 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-xs text-[#E6EAF0]/40">
...
import { Facebook, Twitter, Instagram, Linkedin, Mail } from 'lucide-vue-next'
...
const quickLinks = ['Inicio', 'Agenda', 'Speakers', 'Galería', 'Tickets']
</script>
