<template>
  <section id="gallery" class="py-24 md:py-32 bg-[#050810] relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
      ...
      <div class="flex flex-col items-center text-center mb-16 md:mb-24" v-motion :initial="{ opacity: 0, y: 30 }"
        ...
        <div
          ...
          <span class="w-1.5 h-1.5 bg-[#00F5D4] rounded-full animate-pulse shadow-[0_0_10px_#00F5D4]"></span>
          <span class="text-[#00F5D4] font-orbitron font-bold text-xs tracking-[0.25em] uppercase">Memoria y
            Legado</span>
          <span class="w-1.5 h-1.5 bg-[#00F5D4] rounded-full animate-pulse shadow-[0_0_10px_#00F5D4]"></span>
        </div>
        ...
        <h2
          ...
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#00F5D4] to-[#3B82F6] drop-shadow-sm">Del
            Desierto</span>
        </h2>

        <p class="font-inter text-[#E6EAF0]/70 max-w-2xl text-lg md:text-xl font-light">
          Revive los momentos más destacados de nuestra edición pasada en la Estación Experimental Canchones, UNAP,
          Tarapacá. Agosto 2024.
        </p>
      </div>

      <!-- MASONRY GALLERY -->
      <div class="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6 mb-32">
        <div v-for="(image, i) in images" :key="i" @click="openLightbox(i)" v-motion
          :initial="{ opacity: 0, scale: 0.95 }"
          :visibleOnce="{ opacity: 1, scale: 1, transition: { duration: 600, delay: (i % 6) * 100 } }"
          ...
          <img :src="image" :alt="`Gallery Edition 2024 - ${i + 1}`"
          ...
          <div
            ...
            <div
              ...
              <span class="font-orbitron text-white text-sm tracking-widest font-bold">VER CAPTURA</span>
              <div
      ...
      <!-- HEADER: Impacto Real -->
      <div class="flex flex-col items-center text-center mb-16" v-motion :initial="{ opacity: 0, y: 30 }"
        :visibleOnce="{ opacity: 1, y: 0, transition: { duration: 700 } }">
        <h2 class="font-orbitron text-3xl md:text-4xl lg:text-5xl font-black text-white uppercase tracking-tighter">
          Con un <br class="md:hidden" />
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#FF6B00] via-[#FF8C00] to-[#FFC857]">Impacto
            Real</span>
        </h2>
        <div class="w-24 h-1 bg-gradient-to-r from-transparent via-[#FF6B00] to-transparent mt-8"></div>
      </div>

      <!-- STATS GRID -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 relative perspective-[1000px]">
        <div v-for="(stat, index) in stats" :key="index" v-motion :initial="{ opacity: 0, y: 40 }"
          ...
          <div class="relative z-10 flex flex-col items-center">
            <div
              ...
              :class="stat.color">
              <span v-if="stat.prefix" class="text-4xl md:text-5xl mr-1">{{ stat.prefix }}</span>
              {{ displayValues[index] }}
            </div>
            <span
    ...
    <Teleport to="body">
      <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0"
        ...
        <div v-if="selectedImageIndex !== null"
          ...
          <!-- Image container -->
          <div class="relative max-w-[95vw] lg:max-w-6xl max-h-[90vh] w-full px-4 flex justify-center items-center"
            @click.stop>
          ...
          <div class="absolute top-0 bottom-0 left-0 w-1/4 z-[105] md:hidden" @click.stop="prevImage"></div>
          <div class="absolute top-0 bottom-0 right-0 w-1/4 z-[105] md:hidden" @click.stop="nextImage"></div>
        </div>
      </transition>
    </Teleport>
  </section>
...
const stats = [
  { value: 650, prefix: '+', label: 'Asistentes', color: 'text-[#00F5D4]', glowClass: 'bg-[#00F5D4]' },
  { value: 10, prefix: '+', label: 'Días de Actividad', color: 'text-[#FF6B00]', glowClass: 'bg-[#FF6B00]' },
  { value: 40, prefix: '', label: 'Expertos Globales', color: 'text-[#FFC857]', glowClass: 'bg-[#FFC857]' },
  { value: 50, prefix: '', label: 'Proyectos Lanzados', color: 'text-[#3B82F6]', glowClass: 'bg-[#3B82F6]' },
]
